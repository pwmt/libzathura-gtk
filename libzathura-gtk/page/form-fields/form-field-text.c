...
zathura_gtk_form_field_text_init(ZathuraFormFieldText* widget)
{
  ...
  priv->gtk.single.text_widget = NULL;
    ...
    case ZATHURA_FORM_FIELD_TEXT_TYPE_NORMAL:
      {
        ...
        priv->gtk.single.text_widget = gtk_entry_new();
        gtk_entry_set_has_frame(GTK_ENTRY(priv->gtk.single.text_widget), FALSE);
        gtk_entry_set_max_length(GTK_ENTRY(priv->gtk.single.text_widget), max_length);
        gtk_entry_set_visibility(GTK_ENTRY(priv->gtk.single.text_widget), !is_password);

        g_signal_connect(priv->gtk.single.text_widget, "focus-out-event",
            G_CALLBACK(cb_form_field_text_focus_out),
            widget);

        if (text != NULL) {
          gtk_entry_set_text(GTK_ENTRY(priv->gtk.single.text_widget), text);
        }
        gtk_container_add(GTK_CONTAINER(widget), priv->gtk.single.text_widget);
      }
      break;
    case ZATHURA_FORM_FIELD_TEXT_TYPE_MULTILINE:
      {
        priv->gtk.multiline.text_widget = gtk_text_view_new();
        GtkTextBuffer* buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(priv->gtk.multiline.text_widget));

        if (text != NULL) {
          gtk_text_buffer_set_text(buffer, text, -1);
        }

        g_signal_connect(priv->gtk.multiline.text_widget, "focus-out-event",
            G_CALLBACK(cb_form_field_text_multiline_focus_out),
            widget);

        priv->gtk.multiline.scrolled_window = gtk_scrolled_window_new(NULL, NULL);
        gtk_container_add(GTK_CONTAINER(priv->gtk.multiline.scrolled_window), priv->gtk.multiline.text_widget);
        gtk_container_add(GTK_CONTAINER(widget), priv->gtk.multiline.scrolled_window);
      }
      break;
    ...
    ZathuraFormFieldText* form_field_widget)
{
  ...
  if (zathura_form_field_text_set_text(priv->form_field, gtk_entry_get_text(GTK_ENTRY(priv->gtk.single.text_widget))) != ZATHURA_ERROR_OK) {
    return FALSE;
  }
